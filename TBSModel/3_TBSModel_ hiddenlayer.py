...
with tf.name_scope("layer1") as scope:
	...
	W1 = tf.get_variable("W1", shape=[40, 100],
...
with tf.name_scope("layer2") as scope:
	W2 = tf.get_variable("W2", shape=[100, 200],
                     initializer=tf.contrib.layers.xavier_initializer())
...
with tf.name_scope("layer3") as scope:
	W3 = tf.get_variable("W3", shape=[200, 100],
                     initializer=tf.contrib.layers.xavier_initializer())
...
with tf.name_scope("layer4") as scope:
	W4 = tf.get_variable("W4", shape=[100, 10],
                     initializer=tf.contrib.layers.xavier_initializer())
	b4 = tf.Variable(tf.random_normal([10]))
	L4 = tf.sigmoid(tf.matmul(L3, W4) + b4)

	w4_hist = tf.summary.histogram("weights3",W4)
	b4_hist = tf.summary.histogram("biases3",b4)
	L4_hist = tf.summary.histogram("layer3", L4);
...
with tf.name_scope("layer5") as scope:
	...
	w5_hist = tf.summary.histogram("weights8",W5)
	b5_hist = tf.summary.histogram("biases8",b5)
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter("./logs/TBS3")
