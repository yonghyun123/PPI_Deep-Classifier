...
with open("BIFtrain.csv") as f:
    ncols = len(f.readline().split(','))


xy = np.loadtxt("BIFtrain.csv", skiprows=1, usecols=range(1,ncols), delimiter=',', dtype=np.float32)
...
X = tf.placeholder(tf.float32, [None, 463])
Y = tf.placeholder(tf.float32, [None, 1])


with tf.name_scope("layer1") as scope:

	W1 = tf.get_variable("W1", shape=[463, 512],
...
with tf.name_scope("layer5") as scope:
	...
	W5 = tf.get_variable("W5", shape=[128, 1],
                     initializer=tf.contrib.layers.xavier_initializer())
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter("./logs/BIF3")
